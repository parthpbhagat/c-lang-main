...
public:
    // Default Constructor
    Train() : trainNo(0), trainName(""), source(""), destination(""), seats(0),traintime("") {}

    // Parameterized Constructor
    Train(int no, string name, string src, string dest, int seatCount,string time) {
        trainNo = no;
        trainName = name;
        source = src;
        destination = dest;
...
public:
    ...
    void addTrain() {
        ...
        trains[3] = Train(no, name, src, dest, seats,time);
        count++;
